...
// TODO: Fix tests
    ...
    public void setup() {
        ...
        JdbcTestUtils.deleteFromTables(jdbcTemplate, "user_resetpassword_codes", "users");
    ...
    public void testCreateTokenWhenNonexisting() {
        final String token = resetPasswordTokenDao.generateToken(ID1);
        Assert.assertNotNull(token);
    }

    @Test
    public void testCreateTokenWhenExisting() {
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID1 + ", '" + TOKEN1 + "', '" + Timestamp.valueOf(LocalDateTime.now().plusDays(1)) + "')");

        final String token = resetPasswordTokenDao.generateToken(ID1);

        Assert.assertNotNull(token);
        Assert.assertEquals(token, jdbcTemplate.queryForObject("SELECT code FROM user_resetpassword_codes WHERE user_id = " + ID1, String.class));
    }

    @Test
    public void testCreateTokenWhenExpired() {
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID1 + ", '" + TOKEN1 + "', '" + Timestamp.valueOf(LocalDateTime.now().minusDays(1)) + "')");
    ...
    public void testHasActiveTokenExisting() {
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID1 + ", '" + TOKEN1 + "', '" + Timestamp.valueOf(LocalDateTime.now().plusDays(1)) + "')");
        Assert.assertTrue(resetPasswordTokenDao.hasActiveToken(ID1));
    }

    @Test
    public void testHasActiveTokenNonexisting() {
        Assert.assertFalse(resetPasswordTokenDao.hasActiveToken(ID1));
    }
    ...
    public void testDeleteStaledNoExpiredTokens() {
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID1 + ", '" + TOKEN1 + "', '" + Timestamp.valueOf(LocalDateTime.now().plusDays(1)) + "')");
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID2 + ", '" + TOKEN2 + "', '" + Timestamp.valueOf(LocalDateTime.now().plusDays(1)) + "')");
        resetPasswordTokenDao.deleteStaledTokens();
        Assert.assertEquals(2, jdbcTemplate.queryForObject("SELECT COUNT(*) FROM user_resetpassword_codes WHERE expires > now()", Integer.class).intValue());
    }
    ...
    public void testDeleteStaledSingleToken() {
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID1 + ", '" + TOKEN1 + "', '" + Timestamp.valueOf(LocalDateTime.now().minusDays(1)) + "')");
        resetPasswordTokenDao.deleteStaledTokens();
        Assert.assertFalse(jdbcTemplate.queryForObject("SELECT EXISTS(SELECT * FROM user_resetpassword_codes WHERE expires <= now())", Boolean.class));
    }

    @Test
    public void testDeleteStaledTwoExpiredTokens() {
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID1 + ", '" + TOKEN1 + "', '" + Timestamp.valueOf(LocalDateTime.now().minusDays(1)) + "')");
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID2 + ", '" + TOKEN2 + "', '" + Timestamp.valueOf(LocalDateTime.now().minusDays(1)) + "')");
        resetPasswordTokenDao.deleteStaledTokens();
        Assert.assertFalse(jdbcTemplate.queryForObject("SELECT EXISTS(SELECT * FROM user_resetpassword_codes WHERE expires <= now())", Boolean.class));
    }
    ...
    public void testDeleteStaledOneExpiredOneValid() {
        ...
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID2 + ", '" + TOKEN2 + "', '" + Timestamp.valueOf(LocalDateTime.now().plusDays(1)) + "')");
        resetPasswordTokenDao.deleteStaledTokens();
        Assert.assertFalse(jdbcTemplate.queryForObject("SELECT EXISTS(SELECT * FROM user_resetpassword_codes WHERE user_id = " + ID1 + ")", Boolean.class));
        Assert.assertTrue(jdbcTemplate.queryForObject("SELECT EXISTS(SELECT * FROM user_resetpassword_codes WHERE user_id = " + ID2 + ")", Boolean.class));
    }

    @Test
    public void testIsValidTokenNoTokens() {
        Assert.assertFalse(resetPasswordTokenDao.isValidToken(TOKEN1));
        Assert.assertFalse(resetPasswordTokenDao.isValidToken(TOKEN2));
    }

    @Test
    public void testIsValidTokenOneValidOneNonexisting() {
        jdbcTemplate.execute("INSERT INTO user_resetpassword_codes (user_id, code, expires) VALUES (" + ID1 + ", '" + TOKEN1 + "', '" + Timestamp.valueOf(LocalDateTime.now().plusDays(1)) + "')");
        Assert.assertTrue(resetPasswordTokenDao.isValidToken(TOKEN1));
        Assert.assertFalse(resetPasswordTokenDao.isValidToken(TOKEN2));
    }
    ...
    public void testDeleteGetIdWhenExisting() {
        ...
        Optional<Long> userId = resetPasswordTokenDao.deleteTokenAndRetrieveUserId(TOKEN1);
    ...
    public void testDeleteGetIdWhenOneValidOneNonexisting() {
        ...
        Optional<Long> userId1 = resetPasswordTokenDao.deleteTokenAndRetrieveUserId(TOKEN1);
        Optional<Long> userId2 = resetPasswordTokenDao.deleteTokenAndRetrieveUserId(TOKEN2);
